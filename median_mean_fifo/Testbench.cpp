    ...
    for (x = 0; x != width; ++x) {
      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          ...
          o_g.write(G);
          o_b.write(B);
          //std::cout << "Size == " << o_r.num_free() << std::endl;
      ...
      total[0] = i_result_r.read();
      total[1] = i_result_g.read();
      total[2] = i_result_b.read();

      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = total[0];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = total[1];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = total[2];
